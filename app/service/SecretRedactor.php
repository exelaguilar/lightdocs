...
final class SecretRedactor
{
    ...
    public function redact(string $contents): array
    {
        ...
        $secretKey = '(?:[A-Z0-9_]*(?:SECRET|TOKEN|PASSWORD|PASSWD|PASSPHRASE|API_KEY|PRIVATE_KEY|CLIENT_SECRET|ACCESS_KEY|CREDENTIAL)[A-Z0-9_]*|secret|token|password|passwd|passphrase|api[_-]?key|private[_-]?key|client[_-]?secret|access[_-]?key|credential)';
        $contents = preg_replace_callback(
            '/^([ \t]*(?!contains_secrets\b)' . $secretKey . '[ \t]*[:=][ \t]*)(?![|>][-+]?[ \t]*(?=\r?$))([^\r\n#][^\r\n]*?)(?=\r?$)/im',
