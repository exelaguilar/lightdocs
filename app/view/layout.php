...
$renderTree = function (array $nodes, string $current) use (&$renderTree, $e): string {
    ...
    foreach ($nodes as $node) {
        ...
        if ($node['type'] === 'folder') {
            ...
            $html .= $renderTree($node['children'], $current) . '</details></li>';
        } else {
...
$pageTitle = $title === $config['name'] ? $title : $title . ' · ' . $config['name'];
$initial = mb_strtoupper(mb_substr($config['name'], 0, 1));
$assetVersion = static fn (string $asset): string => $asset . '?v=' . (@filemtime(dirname(__DIR__, 2) . '/public' . $asset) ?: 1);
?>
<!doctype html>
...
<head>
  ...
  <?php if ($canonicalPath !== '' && $config['base_url'] !== ''): ?><link rel="canonical" href="<?= $e($config['base_url'] . $canonicalPath) ?>"><?php endif; ?>
  <link rel="icon" href="/favicon.svg" type="image/svg+xml">
  <link rel="stylesheet" href="<?= $e($assetVersion('/assets/app.css')) ?>">
  ...
  <script type="module" src="<?= $e($assetVersion('/assets/app.js')) ?>"></script>
</head>
<body>
...
<div class="docs-shell">
  ...
  <aside id="sidebar" class="sidebar" aria-label="Documentation navigation">
    ...
    <details class="section-menu" data-section-menu>
      <summary aria-label="Change documentation section"><span class="section-icon icon-<?= $e($currentSection['icon'] ?? 'home') ?>" aria-hidden="true"></span><span class="section-menu-label"><?= $e($currentSection['title'] ?? 'Overview') ?></span><span class="nav-chevron" aria-hidden="true"></span></summary>
      <nav aria-label="Documentation sections">
        <a href="/" <?= $currentSection === null ? 'aria-current="page"' : '' ?>><span class="section-icon icon-home" aria-hidden="true"></span><span><strong>Overview</strong><small><?= $e($config['tagline']) ?></small></span></a>
        <?php foreach ($sections as $section): ?><a href="<?= $e($section['url']) ?>" <?= ($currentSection['path'] ?? '') === $section['path'] ? 'aria-current="page"' : '' ?>><span class="section-icon icon-<?= $e($section['icon']) ?>" aria-hidden="true"></span><span><strong><?= $e($section['title']) ?></strong><?php if ($section['description']): ?><small><?= $e($section['description']) ?></small><?php endif; ?></span></a><?php endforeach; ?>
      </nav>
    </details>
    <?php endif; ?>
    <nav class="sidebar-nav"><?= $renderTree($tree, $currentUrl) ?></nav>
