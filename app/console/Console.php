...
namespace Lightdocs\App\Console;
...
final class Console
{
    ...
    public function __construct(private readonly array $config)
    {
        ...
        $recordBuilder = new SearchIndexer($this->repository, $renderer, $config['cache_dir'] . '/search-index.json');
        ...
        $this->search = new SqliteSearchService($database, $events, $index, $recordBuilder);
        $this->builder = new StaticSiteBuilder($config, $this->repository, $renderer, $this->search, new View(dirname(__DIR__) . '/view'));
    }
    ...
    private function doctor(): int
    {
        ...
        if ($this->config['editor_enabled']) {
            ...
            $checks['Writable environment file'] = is_file($this->config['environment_file']) && is_writable($this->config['environment_file']);
        }
        try {
            (new Database($this->config['database_path']))->connection()->query('SELECT 1')->fetchColumn();
    ...
    private function build(array $arguments): int
    {
        ...
        $built = $this->builder->build($destination, $profile, $acknowledgeSecrets);
