...
final class ContentIndex extends Model
{
    ...
    public function search(string $query, bool $includePrivate = false, int $limit = 20): array
    {
        ...
        $privacy = $includePrivate ? '' : " AND d.visibility = 'public' AND d.draft = 0";
        if ($this->hasFts()) {
    ...
    /** @return list<array<string,mixed>> */
    public function records(bool $includePrivate = false): array
    {
        $this->sync();
        $privacy = $includePrivate ? '' : "WHERE d.visibility = 'public' AND d.draft = 0";
        ...
        $sql = "SELECT d.url AS page, d.title AS description, h.anchor, h.title, h.level, d.type, d.keywords
                FROM headings h JOIN documents d ON d.id = h.document_id " . ($includePrivate ? '' : "WHERE d.visibility = 'public' AND d.draft = 0 ") . 'ORDER BY d.title, h.position';
    ...
    public function saveStudioState(string $sessionId, array $state): void
    {
        ...
        $statement->execute(['id' => $sessionId, 'state' => json_encode($state, JSON_THROW_ON_ERROR), 'time' => time()]);
    }
    ...
    private function indexPage(Page $page): void
    {
        ...
        $statement->execute([
            'path' => $page->relativePath, 'url' => $page->url, 'title' => $page->title, 'description' => $page->description,
            ...
            'hash' => hash('sha256', $page->markdown), 'plain' => $rendered->plainText, 'modified' => $page->modifiedAt, 'indexed' => time(),
        ]);
        $documentId = (int) $this->pdo->lastInsertId();
        $keyword = $this->pdo->prepare('INSERT OR IGNORE INTO keywords(name,slug) VALUES(?,?)');
        $keywordId = $this->pdo->prepare('SELECT id FROM keywords WHERE name = ? COLLATE NOCASE');
        $attachKeyword = $this->pdo->prepare('INSERT OR IGNORE INTO document_keywords(document_id,keyword_id) VALUES(?,?)');
        foreach ($page->keywords() as $name) {
            ...
            $keywordId->execute([$name]);
            $id = $keywordId->fetchColumn();
            if ($id === false) {
                $keyword->execute([$name, $slug . '-' . substr(hash('sha256', $name), 0, 7)]);
                $keywordId->execute([$name]);
                $id = $keywordId->fetchColumn();
            }
            if ($id !== false) $attachKeyword->execute([$documentId, (int) $id]);
        }
        $alias = $this->pdo->prepare('INSERT INTO aliases(document_id,alias) VALUES(?,?)');
        foreach ($page->aliases() as $path) $alias->execute([$documentId, $path]);
        ...
        foreach ($rendered->headings as $position => $item) {
            $heading->execute([$documentId, $item['id'], $item['title'], $item['level'], $position]);
        }
        ...
        foreach ($matches as $match) {
            ...
            $link->execute([$documentId, $target, $kind, $match[1]]);
        }
        ...
        foreach ($includes[1] ?? [] as $path) {
            $usage->execute([str_replace('\\', '/', $path), $documentId]);
        }
        $assetUsage = $this->pdo->prepare('INSERT OR IGNORE INTO asset_usage(asset_path,document_id) VALUES(?,?)');
        preg_match_all('#(?:\(|src=["\'])/uploads/([^\s)"\']+)#', $page->markdown, $assets);
        foreach ($assets[1] ?? [] as $path) {
            $assetUsage->execute([rawurldecode($path), $documentId]);
        }
    }

    private function indexSnippets(): void
    {
        $folder = $this->contentDir . '/_snippets';
        $statement = $this->pdo->prepare('INSERT INTO snippets(path,title,content_hash,modified_at) VALUES(?,?,?,?)');
        foreach (glob($folder . '/*.md') ?: [] as $path) {
            $relative = str_replace('\\', '/', substr($path, strlen($this->contentDir) + 1));
    ...
    {
        if (!is_dir($this->uploadDir)) return;
        $statement = $this->pdo->prepare('INSERT INTO assets(path,url,mime,size,modified_at) VALUES(?,?,?,?,?)');
        $iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($this->uploadDir, FilesystemIterator::SKIP_DOTS));
        $finfo = new \finfo(FILEINFO_MIME_TYPE);
        foreach ($iterator as $item) {
            if (!$item->isFile()) continue;
            if (str_starts_with($item->getFilename(), '.')) continue;
            $relative = str_replace('\\', '/', substr($item->getPathname(), strlen($this->uploadDir) + 1));
    ...
    private function fingerprint(): string
    {
        ...
        $sources = [];
        foreach ([$this->contentDir, $this->uploadDir] as $folder) {
