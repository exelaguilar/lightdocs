    ...
    <form method="post" class="editor-form" data-editor-form data-content-kind="<?= $selected_is_snippet ? 'snippet' : 'page' ?>">
      ...
      <?php else: ?>
        <section class="metadata-panel" data-metadata-panel>
          ...
          <div class="metadata-compact"><label>Order<input type="number" value="<?= $e($editor_meta['order'] ?? 100) ?>" data-meta-field="order"></label><label>Status<select data-meta-field="status"><option value="draft" <?= ($editor_meta['status'] ?? (!empty($editor_meta['draft']) ? 'draft' : 'published')) === 'draft' ? 'selected' : '' ?>>Draft</option><option value="review" <?= ($editor_meta['status'] ?? '') === 'review' ? 'selected' : '' ?>>In review</option><option value="published" <?= ($editor_meta['status'] ?? (!empty($editor_meta['draft']) ? 'draft' : 'published')) === 'published' ? 'selected' : '' ?>>Published</option><option value="archived" <?= ($editor_meta['status'] ?? '') === 'archived' ? 'selected' : '' ?>>Archived</option></select></label><label>Visibility<select data-meta-field="visibility"><option value="public" <?= ($editor_meta['visibility'] ?? 'public') === 'public' ? 'selected' : '' ?>>Public</option><option value="private" <?= ($editor_meta['visibility'] ?? '') === 'private' ? 'selected' : '' ?>>Private</option></select></label><label>Type<select data-meta-field="type"><option value="article" <?= ($editor_meta['type'] ?? 'article') === 'article' ? 'selected' : '' ?>>Article</option><option value="runbook" <?= ($editor_meta['type'] ?? '') === 'runbook' ? 'selected' : '' ?>>Runbook</option></select></label><label>Publish at<input type="datetime-local" value="<?= $e(str_replace(' ', 'T', substr((string) ($editor_meta['publish_at'] ?? ''), 0, 16))) ?>" data-meta-field="publish_at"></label><label>Reviewed<input type="date" value="<?= $e($reviewed_value) ?>" data-meta-field="reviewed"></label><label>Review after<input type="number" min="1" value="<?= $e($editor_meta['review_after'] ?? 180) ?>" data-meta-field="review_after"></label></div>
      ...
      <details class="insert-panel"><summary><span>Insert content</span><small>Directives, page links, and snippets</small><span class="nav-chevron" aria-hidden="true"></span></summary><section class="authoring-toolbar" aria-label="Insert documentation content"><label>Directive<select data-insert-directive><option value="callout">Callout</option><option value="banner">Banner</option><option value="tabs">Tabs</option><option value="filetree">File tree</option><option value="figure">Figure</option><option value="inline-toc">Inline TOC</option><option value="code">Code frame</option><option value="comparison">Comparison</option><option value="details">Details</option></select></label><button type="button" data-insert-directive-button>Insert</button><label>Page link<select data-insert-page><?php foreach ($files as $file): ?><option value="<?= $e($file) ?>"><?= $e($file) ?></option><?php endforeach; ?></select></label><button type="button" data-insert-page-button>Insert</button><label>Snippet<select data-insert-snippet><?php foreach ($snippets as $snippet): ?><option value="<?= $e($snippet['path']) ?>"><?= $e($snippet['title']) ?></option><?php endforeach; ?></select></label><button type="button" data-insert-snippet-button>Include</button><button type="button" data-open-asset-picker>Image</button></section></details>

      <dialog class="asset-picker-dialog" data-asset-picker aria-label="Choose an image">
        <header><div><strong>Insert image</strong><small>Type <code>@image</code> in Markdown to open this picker.</small></div><button type="button" data-close-asset-picker aria-label="Close image picker">×</button></header>
        <label class="asset-picker-search"><span class="sr-only">Search images</span><input type="search" placeholder="Search uploaded images" data-asset-picker-search autocomplete="off"></label>
        <div class="asset-picker-grid" data-asset-picker-grid>
          <?php foreach ($assets as $asset): ?>
            <?php if ($asset['width'] !== null): ?><button type="button" data-asset-picker-item data-asset-url="<?= $e($asset['url']) ?>" data-asset-name="<?= $e($asset['name']) ?>" data-asset-search="<?= $e(mb_strtolower($asset['name'])) ?>"><img src="<?= $e($asset['url']) ?>" alt=""><span><?= $e($asset['name']) ?></span></button><?php endif; ?>
          <?php endforeach; ?>
          <?php if (!array_filter($assets, static fn (array $asset): bool => $asset['width'] !== null)): ?><p class="asset-picker-empty">Upload an image in the Media library first.</p><?php endif; ?>
        </div>
      </dialog>
