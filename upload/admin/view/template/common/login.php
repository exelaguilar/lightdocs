...
<main class="grid w-[min(64rem,100%)] grid-cols-[minmax(0,1.05fr)_minmax(20rem,0.95fr)] overflow-hidden rounded-lg border border-border bg-card text-sm shadow-sm max-[760px]:grid-cols-1">
  ...
  <section class="grid gap-4 rounded-lg border border-border bg-card p-5 text-card-foreground shadow-sm">
    ...
    <form method="post" class="grid gap-3">
      ...
      <div role="group" class="grid min-w-0 gap-1.5"><div class="flex items-center justify-between gap-2"><label for="login-password"><?= $e($text['text_login_password']) ?></label><a class="text-xs text-muted-foreground hover:text-foreground" href="/admin/forgot"><?= $e($text['text_login_forgot_link']) ?></a></div><input class="min-h-9 w-full rounded-md border border-input bg-card px-2.5 py-2 text-sm text-foreground" id="login-password" type="password" name="password" required autocomplete="current-password"></div>
