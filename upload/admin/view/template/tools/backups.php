...
<main class="admin-dashboard">
	...
	<div class="management-layout">
		<section class="dashboard-panel management-panel backup-panel">
			...
			<form method="post" class="backup-create-form"><input type="hidden" name="csrf" value="<?= $e($csrf) ?>"><label><span>Backup label</span><input name="label" value="manual" maxlength="40" required></label><button class="button" type="submit">Create backup</button></form>
		</section>
	...
	<section class="dashboard-panel management-panel backup-list-panel"><header><div><p class="panel-eyebrow">Available archives</p><h2><?= count($archives) ?> backup<?= count($archives) === 1 ? '' : 's' ?></h2></div></header>
		<?php if (!$archives): ?><div class="table-empty">No backups have been created yet.</div><?php else: ?><div class="admin-table-wrap table-borderless"><table class="admin-table"><thead><tr><th>Archive</th><th>Created</th><th>Size</th><th class="table-actions">Action</th></tr></thead><tbody><?php foreach ($archives as $archive): ?><tr><td><strong><?= $e($archive['file']) ?></strong></td><td><?= $e(date('Y-m-d H:i:s', $archive['created_at'])) ?></td><td><?= $e(number_format($archive['size'])) ?> bytes</td><td class="table-actions"><a class="table-link" href="/admin/backups/download?file=<?= rawurlencode($archive['file']) ?>">Download</a></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
	</section>
</main>
</body>
</html>
