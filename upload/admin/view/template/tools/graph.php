...
<main class="admin-dashboard">
  ...
  <div class="graph-grid"><?php foreach ($relationships as $item): ?><article class="graph-card <?= !$item['incoming'] && $item['page']->url !== '/' ? 'is-orphan' : '' ?>"><header><div><span><?= $e($item['page']->relative_path) ?></span><strong><?= $e($item['page']->title) ?></strong></div><a href="/admin?file=<?= rawurlencode($item['page']->relative_path) ?>">Edit</a></header><div><section><span>Incoming <?= count($item['incoming']) ?></span><?php foreach ($item['incoming'] as $page): ?><a href="/admin?file=<?= rawurlencode($page->relative_path) ?>"><?= $e($page->title) ?></a><?php endforeach; ?><?php if (!$item['incoming']): ?><small>No incoming links</small><?php endif; ?></section><section><span>Outgoing <?= count($item['outgoing']) ?></span><?php foreach ($item['outgoing'] as $page): ?><a href="/admin?file=<?= rawurlencode($page->relative_path) ?>"><?= $e($page->title) ?></a><?php endforeach; ?><?php if (!$item['outgoing']): ?><small>No outgoing links</small><?php endif; ?></section></div></article><?php endforeach; ?></div>
</main>
</body>
</html>
