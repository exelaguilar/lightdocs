...
<main class="admin-dashboard">
	...
	<section class="dashboard-panel extension-install-panel"><header><div><p class="panel-eyebrow">Install package</p><h2>Add an extension</h2><p>Upload a ZIP containing an extension manifest and PHP files. Bundled extensions cannot be replaced or removed here.</p></div></header><form method="post" enctype="multipart/form-data" class="extension-install-form"><input type="hidden" name="csrf" value="<?= $e($csrf) ?>"><input type="hidden" name="action" value="install"><input type="file" name="extension" accept=".zip,application/zip" required><button class="button" type="submit">Install extension</button></form></section>
