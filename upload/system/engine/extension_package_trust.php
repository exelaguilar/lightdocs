...
use System\Library\ExtensionSignatureVerifier;

final class ExtensionPackageTrust
{
	...
	public function __construct(string $mode = self::ALLOW_UNSIGNED, array $publicKeys = [], ?callable $verifier = null)
	{
		...
		$this->verifier = $verifier ?? static fn (string $message, string $signature, string $publicKey, string $algorithm): bool => (new ExtensionSignatureVerifier())->verify($message, $signature, $publicKey, $algorithm);
	}

	public function assertTrusted(string $archiveSha256, ?ExtensionPackageProof $proof): void
	{
