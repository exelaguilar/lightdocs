...
final class ContentRepository
{
	...
	public function all(bool $include_drafts = false, bool $include_private = false): array
	{
		$this->pages ??= $this->scan();

		return array_filter($this->pages, static function (Page $page) use ($include_drafts, $include_private): bool {
			return ($include_drafts || !$page->isDraft()) && ($include_private || !$page->isPrivate());
		});
	}

	public function find(string $url, bool $include_drafts = false, bool $include_private = false): ?Page
	{
		$path = '/' . trim(rawurldecode(parse_url($url, PHP_URL_PATH) ?: '/'), '/');
		$path = $path === '/' ? '/' : rtrim($path, '/');
		$pages = $this->all($include_drafts, $include_private);

		return $pages[$path] ?? null;
	}

	public function aliasTarget(string $url, bool $include_private = false): ?Page
	{
		$path = '/' . trim(rawurldecode(parse_url($url, PHP_URL_PATH) ?: '/'), '/');
		$path = $path === '/' ? '/' : rtrim($path, '/');
		return $this->aliasMap($include_private)[$path] ?? null;
	}

	/** @return array<string,Page> */
	public function aliasMap(bool $include_private = false): array
	{
		$pages = $this->all(false, $include_private);
		$aliases = [];
	...
	public function sections(): array
	{
		...
		foreach ($configured as $item) {
			...
			$section_path = trim(str_replace('\\', '/', (string) ($item['path'] ?? '')), '/');
			if ($section_path === '' || str_contains($section_path, '..')) continue;
			$sections[] = [
				'path' => $section_path,
				'title' => (string) ($item['title'] ?? $this->humanize($section_path)),
	...
	public function tree(bool $include_drafts = false, bool $include_private = false): array
	{
		$root = [];
		foreach ($this->all($include_drafts, $include_private) as $page) {
			if (!$page->isInNavigation() && !$include_drafts) {
				continue;
			}
			...
			foreach (array_slice($segments, 0, -1) as $segment) {
				...
				if (!isset($cursor[$segment])) {
					$meta = $this->folderMeta(trim($folder_path, '/'));
					$cursor[$segment] = [
						'type' => 'folder',
						'key' => $segment,
					...
					throw new RuntimeException('A page route conflicts with a documentation folder: ' . $folder_path);
				}
				$cursor =& $cursor[$segment]['children'];
			}
			$key = $segments === [] ? '__root' : end($segments);
			$is_folder_index = $segments !== [] && strtolower(basename($page->relative_path)) === 'index.md';
			if ($is_folder_index) {
				...
				$cursor[$key] = [
					...
					'relativePath' => $page->relative_path, 'draft' => $page->isDraft(), 'private' => $page->isPrivate(), 'children' => $children,
				];
				continue;
			}
			$cursor[$key] = [
	...
	/** @return list<Page> */
	public function orderedPages(bool $include_private = false): array
	{
		$by_url = $this->all(false, $include_private);
		$result = [];
		$walk = function (array $nodes) use (&$walk, &$result, $by_url): void {
			foreach ($nodes as $node) {
				if ($node['type'] === 'page' && isset($by_url[$node['url']])) {
					$result[] = $by_url[$node['url']];
				}
				if ($node['type'] === 'folder') {
					if (isset($by_url[$node['url']])) {
						$result[] = $by_url[$node['url']];
					}
					$walk($node['children']);
				}
		...
		$walk($this->tree(false, $include_private));
		$unique = [];
	...
	public function neighbours(Page $page, bool $include_private = false): array
	{
		$pages = $this->orderedPages($include_private);
	...
	private function scan(): array
	{
		...
		foreach ($iterator as $file) {
			...
			if (isset($pages[$url])) {
				throw new RuntimeException("Duplicate route {$url}: {$relative} and {$pages[$url]->relative_path}");
			}
	...
	{
		$without_extension = substr($relative, 0, -3);
		$segments = explode('/', $without_extension);
	...
	/** @return list<Page> */
	public function backlinks(Page $target, bool $include_private = false): array
	{
		$result = [];
		foreach ($this->all(false, $include_private) as $page) {
	...
	/** @return list<Page> */
	public function outboundLinks(Page $source, bool $include_private = false): array
	{
		$result = [];
		preg_match_all('/\[[^\]]*\]\(([^)]+\.md(?:#[^)]*)?)\)/', $source->markdown, $matches);
		foreach ($matches[1] ?? [] as $href) {
			if (preg_match('#^https?://#i', $href)) continue;
			$target = $this->find($this->resolveMarkdownUrl($source->url, $href), false, $include_private);
	...
	/** @return list<Page> */
	public function relatedPages(Page $page, bool $include_private = false): array
	{
		$values = $page->meta['related'] ?? [];
		if (is_string($values)) $values = [$values];
		$result = [];
		foreach (is_array($values) ? $values : [] as $url) {
			$target = $this->find((string) $url, false, $include_private);
