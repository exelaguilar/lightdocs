...
final class ContentEditor
{
	public function __construct(
		...
		private readonly string $revision_root,
		private readonly string $upload_root,
		private readonly ?MediaProcessor $media_processor = null,
		private readonly ?AssetStorage $asset_storage = null,
	) {
	}
	...
	public function save(string $relative, string $contents, string $expected_hash = ''): void
	{
		...
		if ($expected_hash !== '' && !hash_equals($expected_hash, hash('sha256', $current))) {
	...
	public function upload(array $file): string
	{
		...
		if (!is_dir($this->upload_root)) {
			mkdir($this->upload_root, 0775, true);
		}
		...
		if (!move_uploaded_file((string) $file['tmp_name'], $this->upload_root . '/' . $name)) {
			throw new RuntimeException('Could not store the uploaded file.');
		}

		$this->media_processor?->process($this->upload_root . '/' . $name, $mime);
		return $this->asset_storage?->publish($this->upload_root . '/' . $name, $name, $mime) ?: '/uploads/' . $name;
	}
	...
	public function restore(string $relative, string $revision, string $expected_hash): void
	{
		...
		$this->save($relative, (string) file_get_contents($path), $expected_hash);
	}
	...
	private function resolve(string $relative, bool $must_exist): string
	{
		...
		if ($must_exist && is_file($candidate)) {
	...
	{
		return rtrim($this->revision_root . '/' . trim(dirname($relative), '.'), '/');
	}
}
