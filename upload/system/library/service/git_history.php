...
final class GitHistory
{
	...
	public function initialize(string $author_name, string $author_email): void
	{
		$this->assertEnabled();
		if (is_dir($this->root . '/.git')) throw new RuntimeException('This installation already has a local Git repository.');
		$this->validateIdentity($author_name, $author_email);
		$result = $this->run(['git', 'init', '-b', 'main']);
		if ($result['code'] !== 0) {
			$this->mustRun(['git', 'init']);
			$this->mustRun(['git', 'checkout', '-b', 'main']);
		}
		$this->mustRun(['git', 'config', 'user.name', trim($author_name)]);
		$this->mustRun(['git', 'config', 'user.email', trim($author_email)]);
	}

	public function commit(string $message, bool $acknowledge_secret_history): string
	{
		$this->assertEnabled();
		if (!is_dir($this->root . '/.git')) throw new RuntimeException('Initialize the local repository first.');
		if (!$acknowledge_secret_history) throw new RuntimeException('Acknowledge that local Git history may retain private credentials before committing.');
	...
	public function fileHistory(string $relative_path, int $limit = 30): array
	{
		if (!$this->enabled || !is_dir($this->root . '/.git')) return [];
		$path = $this->repositoryPath($relative_path);
	...
	public function fileAtCommit(string $relative_path, string $commit): string
	{
		$this->assertEnabled();
	...
	private function repositoryPath(string $relative_path): string
	{
	...
	{
		if (!$this->enabled) throw new RuntimeException('Enable Git history in Local Git extension settings first.');
