...
final class Extension implements Contract, AssetStorage
{
	...
	public function publish(string $path, string $name, string $mime): ?string
	{
		...
		$result = (new Http())->put($url, $contents, [
			'Host: ' . $host . $port,
			'Content-Type: ' . $mime,
			'x-amz-content-sha256: ' . $payload_hash,
			'x-amz-date: ' . $amz_date,
			'Authorization: ' . $authorization,
		], max(1, min(120, (int) ($settings['timeout'] ?? 15))));
		if ($result['error'] !== '' || $result['status'] < 200 || $result['status'] >= 300) throw new RuntimeException('The external asset provider rejected the upload.');
